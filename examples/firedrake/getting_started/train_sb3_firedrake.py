...
def train_single_agent(args):
    ...
    defaults = {
        "cylinder": {"Re": 100, "dt": 1e-2, "mesh": "medium"},
        "rotary_cylinder": {"Re": 100, "dt": 1e-2, "mesh": "medium"},
        "pinball": {"Re": 30, "dt": 1e-2, "mesh": "medium"},
        "cavity": {"Re": 7500, "dt": 1e-4, "mesh": "medium"},
        "step": {"Re": 600, "dt": 1e-2, "mesh": "medium"},
    }
    ...
    if args.obs_type is not None:
        ...
        if "probe" in args.obs_type:
            wake_probes = [(x, y) for x in np.linspace(1.0, 8.0, 10) for y in np.linspace(-1.0, 1.0, 5)]
...
Note: Unlike MAIA/Nek, Firedrake runs directly in Python (no MPMD required).
    ...
    parser.add_argument("--reynolds", type=float, default=None, help="Reynolds number (default: env-specific)")
    parser.add_argument(
        "--mesh",
        type=str,
    ...
    parser.add_argument("--dt", type=float, default=None, help="Time step (default: env-specific)")
    parser.add_argument(
        "--obs-type",
        type=str,
        choices=["lift_drag", "stress_sensor", "velocity_probes", "pressure_probes", "vorticity_probes"],
        default="pressure_probes",
        help="Observation type (default: env-specific)",
    )
    parser.add_argument("--num-substeps", type=int, default=1, help="Number of solver steps per action (default: 1)")
