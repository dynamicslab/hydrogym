...
def run_firedrake_test(env_type: str, num_steps: int, num_episodes: int,
                       reynolds: Optional[float],
                       mesh_resolution: Optional[int], seed: Optional[int],
  ...
  callbacks = [
      ...
      LogCallback(
          ...
          [:4],  # Log first 4 probe velocities
          nvals=4,
          interval=1,
  ...
  try:
    ...
    if hasattr(env, 'flow'):
      ...
      if hasattr(env.flow,
                 'checkpoint_path') and env.flow.checkpoint_path is not None:
