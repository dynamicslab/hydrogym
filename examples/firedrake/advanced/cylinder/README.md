...
**Beginners:**
1. `solve-steady.py` - Understand steady states
2. `unsteady.py` - See instability development
3. `run-transient.py` - Basic time integration

**Intermediate (Stability):**
4. `stability.py` - Learn eigenvalue analysis
5. `step_input.py` - System identification basics

**Advanced (Control):**
6. `run-transient.py` - Generate checkpoint
7. `pd-control.py` - Apply feedback control
8. `pd-phase-sweep.py` - Optimize controller

**Research:**
9. `lti_system.py` - Build reduced-order models
10. `pressure-probes.py` - Design sensor arrays
...
**Workflow 1: Stability-Guided Control Design**
```bash
# 1. Compute base flow
python solve-steady.py

# 2. Find unstable eigenvalues/eigenmodes
python stability.py --base-flow output/cylinder_Re100_steady.h5

# 3. Design controller using eigenvalue predictions
python pd-control.py  # (after running run-transient.py)
```

**Workflow 2: System Identification → Model-Based Control**
```bash
# 1. Measure step response
python step_input.py

# 2. Extract linearized operators
python lti_system.py

# 3. Design optimal controller (future: LQR/MPC examples)
```

**Workflow 3: Control Optimization**
```bash
# 1. Generate initial vortex shedding
python run-transient.py

# 2. Sweep controller parameters
python pd-phase-sweep.py

# 3. Apply optimal parameters
python pd-control.py  # (modify gains based on sweep results)
```

### Understanding the Physics

**Reynolds Number Regimes:**
- **Re < 47**: Steady state is stable (no vortex shedding)
  - Example: `step_input.py` uses Re=40
- **Re ≈ 47**: Hopf bifurcation (onset of vortex shedding)
- **Re = 100**: Supercritical (periodic vortex shedding)
  - Most examples use this regime
- **Re > 200**: Transition to 3D turbulence (2D models no longer valid)

**Key Dimensionless Numbers:**
- **Reynolds number:** Re = U∞·D/ν (inertia vs. viscosity)
- **Strouhal number:** St = f·D/U∞ ≈ 0.165 at Re=100 (shedding frequency)
- **Lift coefficient:** CL = F_lift / (0.5·ρ·U∞²·D)
- **Drag coefficient:** CD = F_drag / (0.5·ρ·U∞²·D)

### Tips and Troubleshooting

**Common Issues:**

1. **"Checkpoint file not found" in pd-control.py**
   - Solution: Run `python run-transient.py` first to generate checkpoint

2. **Stability.py convergence problems**
   - Try increasing `--krylov-dim` (more Arnoldi vectors)
   - Use `--sigma 0.1+0.5j` to target specific eigenvalues

3. **Oscillations in solution**
   - Reduce time step `dt`
   - Try `stabilization="gls"` or `"supg"`

4. **Memory issues with fine mesh**
   - Use coarser mesh: `mesh="coarse"`
   - Run with MPI: `mpirun -np 4 python script.py`
...
# Check MPI scaling
mpirun -np 1 python stability.py  # Compare timing
mpirun -np 4 python stability.py  # vs parallel
```

### Further Reading

**Theory:**
- Williamson & Brown (1998) - Vortex shedding review
- Colonius & Taira (2008) - Fast immersed boundary method
- Schmid & Henningson (2001) - Stability and transition

**Control:**
- Protas & Wesfreid (2002) - Cylinder drag reduction
- Bergmann et al. (2005) - Optimal rotary control
