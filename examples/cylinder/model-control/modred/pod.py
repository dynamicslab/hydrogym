...
coeffs, basis_handles = gym.linalg.pod(flow,
    snapshot_handles = [gym.linalg.Snapshot(f'{snap_prefix}{idx}.npy') for idx in range(m)],
...
data_handles = [gym.linalg.Snapshot(f'{transient_prefix}{idx}.npy', base_vec_handle=mean_handle) for idx in range(m)]
