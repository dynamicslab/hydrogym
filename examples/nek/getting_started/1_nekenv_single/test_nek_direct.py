...
def main():
  parser = argparse.ArgumentParser(
      description="Simple Nek5000 test with direct instantiation")
  parser.add_argument(
  ...
  print(
      f"  Action bounds: [{env.action_space.low[0]:.6f}, {env.action_space.high[0]:.6f}]"
  )
  print('=' * 80)
  ...
  for step in range(max_steps):
    ...
    if step % 10 == 0:
      print(
          f"  Step {step:3d}: reward = {reward:8.6f}, total = {total_reward:10.6f}"
      )
