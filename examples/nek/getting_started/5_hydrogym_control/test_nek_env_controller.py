...
def run_nek_test(env_name: Optional[str], nproc: int,
                 config_path: Optional[str], num_steps: Optional[int],
                 num_episodes: int, controller_type: str,
                 local_dir: Optional[str], logger: logging.Logger) -> None:
  """
    ...
    for episode in range(num_episodes):
      ...
      try:
        ...
        for step in range(max_steps):
          ...
          # Get dt from config
          dt = env.conf.simulation.dt if hasattr(env.conf.simulation,
                                                 'dt') else 0.001
          t = step * abs(dt)
...
def main():
  ...
  parser.add_argument(
      ...
      type=int,
      default=None,
      help="Number of steps per episode (overrides config)")
  parser.add_argument(
      "--num-episodes",
      type=int,
      default=1,
