...
def create_environment(
    ...
    if env_name:
        ...
        raise ValueError("Must provide either env_name (MAIA) or config_path (legacy)")
...
def classical_controllers():
    ...
    def sinusoidal_control(t, obs, env):
        """Sinusoidal control."""
        return np.sin(2 * np.pi * t * 0.1) * 0.5 * np.ones(env.action_space.shape, dtype=np.float32)

    return {"opposition": opposition_control, "zero": zero_control, "sinusoidal": sinusoidal_control}
...
def main():
    ...
    parser.add_argument("--config-file", type=str, default=None, help="Override config file path (None = auto-detect)")

    # Algorithm parameters
    parser.add_argument(
        "--algo", default="PPO", choices=["PPO", "TD3", "SAC"], help="RL algorithm to use (default: PPO)"
    )
    parser.add_argument(
        "--total-timesteps", type=int, default=50000, help="Total timesteps for training (default: 50000)"
    )
    parser.add_argument(
        "--n-steps", type=int, default=2048, help="Number of steps per update (PPO only, default: 2048)"
    )
    ...
    # Evaluation
    parser.add_argument("--eval-steps", type=int, default=100, help="Steps for integrate() evaluation (default: 100)")

    # Logging
    parser.add_argument("--log-dir", default="./logs", help="Directory for saving logs and models (default: ./logs)")
