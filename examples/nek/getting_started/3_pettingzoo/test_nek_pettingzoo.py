...
def main():
  parser = argparse.ArgumentParser(
      description="Test Nek5000 PettingZoo multi-agent environment")
  parser.add_argument(
      "--steps", type=int, default=100, help="Number of steps to run")
  parser.add_argument(
      "--env", type=str, default="MiniChannel_Re180", help="Environment name")
  parser.add_argument(
      "--nproc", type=int, default=10, help="Number of Nek5000 processes")
  parser.add_argument(
      "--local-dir",
      type=str,
      default=None,
  ...
  for i, agent_name in enumerate(env.possible_agents[:3]):
    ...
    print(
        f"       act_bounds=[{act_space.low[0]:.6f}, {act_space.high[0]:.6f}]")
  ...
  print(
      f"  First agent '{first_agent[:40]}...' obs shape: {obs_dict[first_agent].shape}"
  )
  ...
  for step in range(max_steps):
    ...
    if step % 10 == 0:
      ...
      print(
          f"  Step {step:3d}: agent[0] reward = {agent_reward:8.6f}, total = {agent_total:10.6f}"
      )
