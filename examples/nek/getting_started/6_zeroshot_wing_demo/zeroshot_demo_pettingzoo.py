...
class MultiPolicyDeployer:
  ...
  def _validate_actuator_info(self):
    if not hasattr(self._base_env, 'actuator_info'):
      raise RuntimeError(
          "Base environment does not expose actuator_info; cannot assign policy regions.")
  ...
  def _load_controller(self, spec: PolicySpec):
    ...
    try:
      ...
      raise RuntimeError(
          f"stable_baselines3 import failed for {algo}. Install SB3 or change algorithm in policy template.") from exc

    model_path = spec.model_path
    if not model_path:
      if not self.policy_root:
        raise ValueError(
            f"Policy '{spec.name}' requires --policy-root or explicit model_path")
      model_path = self.policy_root / spec.agent_run_name / 'logs' / (
          f"{spec.agent_run_name}-{spec.policy}")

    model_path = Path(model_path).expanduser().resolve()
    if not model_path.exists():
      raise FileNotFoundError(
          f"Model file not found for {spec.name}: {model_path}")
  ...
  def _build_groups(self):
    ...
    if unassigned > 0:
      print(
          f"[META] {unassigned} agents are not covered by any policy. "
          "They will receive zero action.",
          flush=True)
  ...
  def print_reward_table(self, step: int):
    ...
    for group in self.groups:
      ...
      print(
          f"| {spec.name:<10s} | alg={spec.algorithm:<4s} | avg_scaled_R={avg:>10.6f} |",
          flush=True)
    print("-" * 56, flush=True)
...
def main():
  parser = argparse.ArgumentParser(
      description="Zero-shot multi-policy wing deployment demo")
  parser.add_argument(
      "--policy-template",
      type=str,
  ...
  parser.add_argument(
      ...
      type=str,
      default=None,
      help="Override policy root folder for RL policies")
  parser.add_argument(
      "--local-dir",
      type=str,
      default=None,
      help="Local fallback directory for environment packages")
  parser.add_argument(
      "--log-every",
      type=int,
      default=100,
      help="Print reward summary every N steps")
