import firedrake as fd
import numpy as np
from firedrake.petsc import PETSc
from ufl import real

import hydrogym as hgym

assert (
    PETSc.ScalarType == np.complex128
), "Complex PETSc configuration required for stability analysis"

output_dir = "output"

# First we have to ramp up the Reynolds number to get the steady state
Re_init = [500, 1000, 2000, 4000]
flow = hgym.flow.Cavity(Re=Re_init[0])
hgym.print(f"Steady solve at Re={Re_init[0]}")
qB = flow.solve_steady(solver_parameters={"snes_monitor": None})

for (i, Re) in enumerate(Re_init[1:]):
    flow.Re.assign(real(Re))
    hgym.print(f"Steady solve at Re={Re_init[i+1]}")
    qB = flow.solve_steady(solver_parameters={"snes_monitor": None})

hgym.print("Starting direct stability analysis")
# First, direct analysis
A, M = flow.linearize(qB)
evals, es = hgym.linalg.eig(A, M, num_eigenvalues=20, sigma=7.5j)
max_idx = np.argmax(np.real(evals))
nconv = es.getConverged()
vr, vi = A.getVecs()
hgym.print(f"Re={Re}:\t\t{nconv} converged, largest: {evals[max_idx]}")

es.getEigenpair(max_idx, vr, vi)
evec_dir = fd.Function(flow.mixed_space)
hgym.utils.set_from_array(evec_dir, vr.array)

pvd = fd.File("output/evec_dir.pvd")
u, p = evec_dir.split()
vort = fd.project(fd.curl(u), flow.pressure_space)
u.rename("u")
p.rename("p")
vort.rename("vort")
pvd.write(u, p, vort)

np.save("output/evals.npy", evals)

# Adjoint
A, M = flow.linearize(qB, adjoint=True)
evals, es = hgym.linalg.eig(A, M, num_eigenvalues=20, sigma=7.5j)
max_idx = np.argmax(np.real(evals))
nconv = es.getConverged()
vr, vi = A.getVecs()
hgym.print(f"Re={Re}:\t\t{nconv} converged, largest: {evals[max_idx]}")

es.getEigenpair(max_idx, vr, vi)
evec_adj = fd.Function(flow.mixed_space)
hgym.utils.set_from_array(evec_adj, vr.array)

pvd = fd.File("output/evec_adj.pvd")
u, p = evec_adj.split()
vort = fd.project(fd.curl(u), flow.pressure_space)
u.rename("u")
p.rename("p")
vort.rename("vort")
pvd.write(u, p, vort)
