...
HydroGym is a reinforcement learning platform for fluid dynamics and active flow control. It provides a unified, [Gymnasium](https://gymnasium.farama.org/)-compatible interface to 88 pre-configured CFD environments that span six solver backends, two to three spatial dimensions, and Reynolds numbers from laminar benchmarks to wall-bounded turbulence above Re = 400 000.

The central design goal is to make it straightforward to apply standard RL tooling — [Stable-Baselines3](https://stable-baselines3.readthedocs.io/), [RLlib](https://docs.ray.io/en/latest/rllib/index.html), [PettingZoo](https://pettingzoo.farama.org/), or your own training loop — to physically meaningful flow control problems without having to write a custom environment or solver interface. Every environment exposes the same `env.reset()` / `env.step()` API; solver-specific setup is handled behind the scenes.

Environment configurations and initial-condition checkpoints are distributed through [HuggingFace Hub](https://huggingface.co/datasets/dynamicslab/HydroGym-environments) and are downloaded automatically the first time an environment is created. Pre-built Docker images are available for all solver backends, making it possible to go from zero to a running training loop in a single `docker pull` command.

The six supported solver backends cover the main families of computational fluid dynamics methods:
...
HydroGym is described in detail in [Lagemann et al. (2025)](https://arxiv.org/abs/2512.17534). If you use it in your research, please cite that work.
