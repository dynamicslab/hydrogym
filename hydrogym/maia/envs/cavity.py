...
class CavityBase(MaiaFlowEnv):
  ...
  def _find_baseline_file(self) -> Optional[str]:
    ...
    print(
        f"WARNING: No baseline state file auto-detected in {self.env_data_path}"
    )
  ...
  def _load_baseline_data(self, env_config: Dict) -> None:
    ...
    if baseline_file.endswith('.feather'):
      print(
          f'Loading baseline data from Feather file: {baseline_file}',
          flush=True)
      ...
      raise ValueError(
          f"Unsupported file format: {baseline_file}. Use .feather or .csv")
    ...
    if len(coord_cols) == 0:
      if all(col in baseline_df.columns
             for col in ['Points:0', 'Points:1', 'Points:2']):
        coord_cols = ['Points:0', 'Points:1', 'Points:2']
      else:
        raise ValueError(f"Could not find coordinate columns in baseline file. "
                         f"Available columns: {baseline_df.columns.tolist()}")

    baseline_coords = baseline_df[coord_cols[:self.nDim]].values

    print(
        f'Building KD-Tree for {len(baseline_coords)} baseline points',
        flush=True)
    kdtree = cKDTree(baseline_coords)

    probe_coords = np.array(self.probe_locations).reshape(-1, self.nDim)
    print(
        f'Finding baseline values for {len(probe_coords)} probe locations',
        flush=True)
    ...
    if 'forces' in self.observation_type:
      print(
          'WARNING: Forces are in observation_type but not extracted from baseline file'
      )
    ...
    print(
        f'Baseline values extracted. Shape: {self.baseline_values.shape}',
        flush=True)
    print(
        ...
        flush=True)
    print(
        f'Max distance to nearest baseline point: {distances.max():.6f}',
        flush=True)
  ...
  def get_reward(self) -> Tuple[float, Dict]:
    ...
    if self.reward_strategy == 'running_mean':
      ...
      self.running_mean = self.alpha * self.obs + (
