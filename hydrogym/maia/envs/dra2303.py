...
This module provides DRA2303 airfoil CFD environments for reinforcement learning
with two simulation scenarios at Ma=0.2 and Ma=0.7 with chord-based Re=400000.
...
class DRA2303Jet(DRA2303Base):
    ...
    and reward computation are all provided by :class:`DRA2303Base`.
...
class DRA2303SurfaceWave(DRA2303Base):
    ...
    * **Reward** – :math:`R = -C_D - \\omega \\cdot C_P` (shared with
      :class:`DRA2303Jet` via :class:`DRA2303Base`; here ``C_P > 0``
    ...
    .. code-block:: yaml
        ...
        maia:
          ...
          omega:               0.1
          wave_zone_ids:       [10, 11]  # pressure side, suction side BC IDs
    """
    ...
    def __init__(self, env_config: Dict):
        ...
        # ------------------------------------------------------------------
        # Wave zone boundary condition IDs
        # ------------------------------------------------------------------
        self.wave_zone_ids = self.cfg.maia.get("wave_zone_ids", None)
        if self.wave_zone_ids is None:
            raise ConfigError(
                "DRA2303SurfaceWave requires 'wave_zone_ids' in config "
                "(list of BC IDs for pressure and suction side wave zones)"
            )
        if len(self.wave_zone_ids) != 2:
            raise ConfigError(
                f"wave_zone_ids must contain exactly 2 BC IDs "
                f"(pressure side and suction side), got {len(self.wave_zone_ids)}"
            )
        print(f"Wave zone BC IDs: pressure={self.wave_zone_ids[0]}, suction={self.wave_zone_ids[1]}")
    ...
    def convert_action(self, action: np.ndarray) -> List[float]:
        ...
        The same physical parameters are applied to both wave zones
    ...
    def step(self, action: Optional[np.ndarray] = None) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        # Apply the same control parameters to both wave zones
        physical_action = self.convert_action(action=action)
        self.maiaInterface.setControlProperties(physical_action)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        # Same parameters for both wave zones.
...
register_environment("DRA2303Jet", DRA2303Jet)
register_environment("DRA2303SurfaceWave", DRA2303SurfaceWave)
