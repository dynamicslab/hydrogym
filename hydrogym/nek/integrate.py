    ...
    while t < t_end and iter < max_steps:
        ...
        if controller is not None:
            ...
            if hasattr(env, "action_space"):
                ...
                action = {
                    agent: np.zeros(env.action_space(agent).shape, dtype=np.float32) for agent in env.possible_agents
                }
        ...
        if vars_record and (iter % vars_record_freq == 0):
            ...
            # Array-based environment (NekEnv)
                ...
                if len(agents_list) == 1:
                    ...
                    obs_per_agent = getattr(env, "per_agent_obs_size", None) or getattr(env, "obs_per_actuator", None)
