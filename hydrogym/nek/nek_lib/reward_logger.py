...
class RewardLogger:
  ...
  def _open_files(self):
    ...
    if self.log_per_agent and self.per_agent_writer is None:
      ...
      # Write header
      header = [
          'timestamp', 'episode', 'step', 'agent_name', 'reward', 'dUdy_raw'
      ]
    ...
    if self.log_aggregated and self.aggregated_writer is None:
      ...
      # Write header
      header = [
          'timestamp', 'episode', 'step', 'mean_reward', 'std_reward',
    ...
    if self.episode_writer is None:
      ...
      # Write header
      header = [
          'timestamp', 'episode', 'total_steps', 'mean_reward', 'std_reward',
  ...
  def log_rewards(self,
    ...
    if self.log_aggregated:
      row = [
          current_time, episode, step, mean_reward, std_reward, min_reward,
          max_reward, total_reward, num_agents
      ]
  ...
  def log_episode_summary(self, episode: Optional[int] = None):
    ...
    # Log episode summary
    row = [
        current_time, episode, total_steps, mean_reward, std_reward, min_reward,
        max_reward, total_reward, duration
    ]
