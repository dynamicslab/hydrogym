...
class NekEnv(gym.Env):
  ...
  def _init_from_legacy(self, conf: Config, run_root: str,
                        run_name: Optional[str], reward_agg: str):
  ...
  def _init_from_hf(self, env_config: Dict, run_root: str,
                    run_name: Optional[str], reward_agg: str):
  ...
  def _create_session_file_early(self):
    ...
    if hasattr(self.conf, 'simulation') and hasattr(self.conf.simulation,
                                                    'CASENAME'):
  ...
  def _update_configuration_paths(self):
    ...
    elif hasattr(self.conf, 'simulation') and hasattr(self.conf.simulation,
                                                      'restart_folder'):
  ...
  def _initialize(self):
    ...
    restart_folder = self._get_config_value(
        ('initial_conditions', 'restart_folder'),
        ('simulation', 'restart_folder'))
    ...
    # Save config
    OmegaConf.save(self.conf, os.path.join(self.history_path,
                                           'current_conf.yml'))
    ...
    self.TOTCTRL = self._get_config_value(('mesh', 'TOTCTRL'),
                                          ('simulation', 'TOTCTRL'))
    ...
    self.utau = self._get_config_value(('normalization', 'u_tau'),
                                       ('runner', 'u_tau'))
    ...
    self.rescale_actions = self._get_config_value(
        ('rl_interface', 'rescale_actions'), ('runner', 'rescale_actions'),
        default=False)
    ctrl_max_amp = self._get_config_value(('rl_interface', 'ctrl_max_amp'),
                                          ('runner', 'ctrl_max_amp'),
                                          default=1.0)
    ctrl_min_amp = self._get_config_value(('rl_interface', 'ctrl_min_amp'),
                                          ('runner', 'ctrl_min_amp'),
                                          default=-1.0)
    ...
    # Reward setup
    self.baseline_dudy = self._get_config_value(('normalization', 'dUdy'),
                                                ('runner', 'dUdy'))
    ...
    rew_mode = self._get_config_value(('episode', 'reward_mode'),
                                      ('runner', 'rew_mode'),
                                      default='Homo')
    if rew_mode == 'MovingAverage':
      size_history = self._get_config_value(('episode', 'size_history'),
                                            ('runner', 'size_history'),
                                            default=10)
    ...
    self.znmf_avg = self._get_config_value(('physics', 'znmf_avg'),
                                           ('simulation', 'znmf_avg'),
                                           default=0)
    self.normalize_input = self._get_config_value(
        ('normalization', 'normalize_input'), ('runner', 'normalize_input'),
        default='None')
    self.random_init = self._get_config_value(
        ('initial_conditions', 'random_init'), ('runner', 'random_init'),
        default=1)
  ...
  def _init_actuators(self):
    ...
    request = b"INTAL"
    self.sub_comm.Send([request, MPI.CHARACTER],
                       dest=0,
    ...
    node_list = np.zeros(
        (self.nproc,),
        dtype=np.int32)  # Initialize to zero to avoid garbage values
    ...
    print(
        f'[NEK] Will receive actuator info from {len(nid_list)} ranks',
        flush=True)
    ...
    for nid in nid_list:
      ...
      for k in self.actuator_info.keys():
        if 'NID' not in k:
          if 'NUMCTRL' not in k:
            rank_data[k] = np.empty((self.TOTCTRL,),
          ...
          print(
              ...
              flush=True)
          self.sub_comm.Recv([rank_data[k], tag_dict[k]['mpi_dtype']],
                             nid,
  ...
  def step(self,
    ...
    if self.act_index >= self.nb_interactions:
      print(
          f'[STEP] ACT_INDEX={self.act_index}; TERMINATED == TRUE', flush=True)
      terminated = True
  ...
  def _get_state(self) -> Tuple[float, np.ndarray]:
    ...
    state_buffer = np.ndarray(
        shape=(self.nNID, NFLDC, TOTCTRL), dtype=np.float64)
    ...
    for il, nid in enumerate(self.uniqID):
      indx = np.where((self.actuator_info['NID'] == nid))[0]
      for jl in range(len(indx)):
    ...
    assert icount == self.n_actuators, ValueError(
        '[NEK] Actuator count mismatch!')
  ...
  def _send_action(self, action: np.ndarray):
    ...
    for il, nid in enumerate(self.uniqID):
      ...
      # Send buffer
      self.sub_comm.Send([act_buffer, tag_dict['ACTION']['mpi_dtype']],
                         nid,
  ...
  def _normalize_state(self, state: np.ndarray) -> np.ndarray:
    ...
    if self.normalize_input != "None":
      if self.normalize_input == "utau":
        ...
        state = 2 * (state - np.min(state)) / (np.max(state) -
                                               np.min(state)) - 1
    return state
