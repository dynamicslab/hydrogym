...
class NekParallelEnv:
  ...
  def observation_space(self, agent):
    ...
    return gym.spaces.Box(
        low=-np.inf, high=np.inf, shape=(self.obs_per_agent,), dtype=np.float32)
  ...
  def action_space(self, agent):
    ...
    return gym.spaces.Box(
        low=self.env.action_space.low[idx],
        high=self.env.action_space.high[idx],
        shape=(1,),
        dtype=np.float32)

  def reset(self,
            seed=None,
            options=None) -> Tuple[Dict[str, np.ndarray], Dict]:
  ...
  def step(
    ...
    if 'reward_per_actuator' in info:
      rewards_dict = {
          agent: float(info['reward_per_actuator'][i])
