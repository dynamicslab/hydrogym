...
class HFDataManager:
  ...
  def __init__(self,
    ...
    if not HF_AVAILABLE:
      self.logger.warning(
          "Hugging Face Hub not available. Using local fallback only.")
  ...
  def _detect_solver_profile(self, env_name: str) -> str:
    ...
    if os.path.exists(local_env_dir):
      for sentinel, profile_name in _SENTINEL_TO_PROFILE.items():
        if os.path.exists(os.path.join(local_env_dir, sentinel)):
          self.logger.info(f"Detected solver profile '{profile_name}' "
    ...
    if HF_AVAILABLE:
      try:
        ...
        for file_path in repo_files:
          ...
          if (len(parts) == 2 and parts[0] == env_name and
              parts[1] in _SENTINEL_TO_PROFILE):
    ...
    # 4. Last resort
    self.logger.info(f"No sentinel found for '{env_name}', "
  ...
  def get_available_environments(self) -> List[str]:
    ...
    if self.local_fallback_dir and os.path.exists(self.local_fallback_dir):
      return [
          d for d in os.listdir(self.local_fallback_dir)
...
#[YW-MOD]
    ...
    for source_name, target_rel in workspace_dirs.items():
      ...
      if source_path.exists():
        ...
        self.logger.warning(
            f"Workspace source dir not found, skipping: {source_path}")
  ...
  def _get_environment_path_with_symlink(self, env_name: str,
                                         force_download: bool,
                                         profile: str) -> str:
    ...
    if self.local_fallback_dir and not force_download:
      ...
      if os.path.exists(
          local_fallback_path) and self._validate_environment_files(
              local_fallback_path, profile):
        print(
            f"Using local fallback directly (skip cache): {local_fallback_path}"
        )
    ...
    if os.path.islink(local_env_link) and not force_download:
      ...
      if os.path.exists(target) and self._validate_environment_files(
          target, profile):
        print(
            f"Using cached environment (symlink): {local_env_link} -> {target}")
  ...
  def _get_environment_path_direct(self, env_name: str, force_download: bool,
    ...
    if HF_AVAILABLE:
      try:
        ...
        if os.path.exists(env_path) and self._validate_environment_files(
            env_path, profile):
  ...
  def _validate_environment_files(self,
    ...
    if solver is None:
      self.logger.warning(
          f"Unknown solver profile '{profile}', skipping validation")
      return True
...
if __name__ == "__main__":
  ...
  if args.list:
    ...
    if args.prepare:
      paths = manager.prepare_working_directory(
          path, args.prepare, profile=args.profile)
