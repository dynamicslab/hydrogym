...
!     text file
 ...
 101        continue
         ...
         if (nid.eq.0) write(*,*) 'npts,nz,dir,z0,dz',pts_npt2d,
     &        pts_nlev, pts_dir, pts_xmin, pts_dx
      else                      ! ifascii
!     binary file
         fname = 'stat_pts.in'//char(0)
cc MA:         call byte_open(fname,ierr)
         call byte_open(fname,ierr,132)
         if (ierr.eq.0) then
!     integer header
...
c-----------------------------------------------------------------------
c     subroutine intpts_setup(tolin,ih)  !!ADDED THIS (Commented, it is in postpro of DEC_Nek5000)
c
...
c      tol = tolin
c      if (tolin.lt.0) tol = 1e-13 ! default tolerance 

c      n       = lx1*ly1*lz1*lelt 
c      npt_max = 256
c      nxf     = 2*nx1 ! fine mesh for bb-test
c      nyf     = 2*ny1
c      nzf     = 2*nz1
c      bb_t    = 0.1 ! relative size to expand bounding boxes by
c
c      if(nidd.eq.0) write(6,*) 'initializing intpts(), tol=', tol
c      call findpts_setup(ih,nekcomm,npp,ndim,
c     &                     xm1,ym1,zm1,nx1,ny1,nz1,
c     &                     nelt,nxf,nyf,nzf,bb_t,n,n,
c     &                     npt_max,tol)
c       
c      return
c      end
