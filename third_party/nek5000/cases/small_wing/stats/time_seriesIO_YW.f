...
cc YW: We wanna sort out those control and wall points       
      integer nptr ! The actual number of points to be saved
      integer ifind ! A flag for finding the points
      integer ilr, jlr         ! Another iteration for ttp 
      real ctx,ptx,wax  ! Current coorindate
      integer ctg,ptg,wag ! Current global points
c-----------------------------------------------------
!     local variables
...
cc YW:
      ! write(*,*) "YW: Local point pts to write: ", npts
...
!     copy vector
         do il=1,npts
            ifind = 0
            ptx = pts(jl,il)
CC YW: Sort out the ctrl and wall points by coordinates
            ! do ilr =1,ttp
            !       do jlr = 1,NDIM
            !             ctx = crdctl(jlr,ilr)
            !             wax = crdwall(jlr,ilr)
            !             ! if ((ctx.eq.ptx).or.(wax.eq.ptx)) then
            !             if ((ctx.eq.ptx).or.(wax.eq.ptx)) then
            !                   ifind = ifind + 1 ! IF find a point meet critera then we sort it out    
            !             endif
            !       enddo 
            ! enddo 
cc YW: Sort out the ctrl and wall points by global points
            ptg = ipts(il)
            do ilr = 1, ttp 
                  ctg = iptctl(ilr)
                  wag = iptwall(ilr)
                  if ((ctg.eq.ptg).or.(wag.eq.ptg)) then
                        ifind = ifind + 1 ! IF find a point meet critera then we sort it out    
                  endif
            enddo 
...
!     offset
         ...
         call stat_mfo_outspts(ur1,nptr)
         ioflds = ioflds + 1
      !    write(*,*) "YW: ldim=",jl
      !    write(*,*) "YW: nptr=", nptr
      enddo ! jl=1,LDIM
...
!     copy vector
            do il=1,npts
                  ...
                  do ilr =1,ttp
                        ctg = iptctl(ilr)
                        wag = iptwall(ilr)
                        if ((ctx.eq.ptg).or.(wag.eq.ptg)) then
                              ifind = ifind + 1
                        endif 
                  enddo 
                  if (ifind.eq.0) then 
                        ur1(nptr) = fieldbuff(jl,il,kl)
                        nptr = nptr +1
                  endif 
            enddo
!     offset
...
!     count bytes
      dnbyte = 1.*ioflds*nptr*WDSIZO
cc ############################ YW modification stop here  ##############################
      ierr = 0
      if (NID.eq.PID0) 
#ifdef MPIIO
...
!     dummy arrays
      ...
      real*4         u4
      real*8         u8(1+lw2)
      equivalence    (u4,u8) ! Share the same memory 
