...
!             endif
! ! ############################################################
! cc YW           Opposition Contrl Sampling , we see the first step as example
!             if (mod(ISTEP,ctrl_comp).eq.0 .and. ISTEP.ge.1) then 
!                   call ctrl_sample
!             endif
! ####################################################################
!           average
...
!     this routine uses number of scratch common blocks
      ...
      if (if3d) then               !#2D
cc UNCOMM FOR STATS:           
           call stat_pts_init    !not for 2D yet !UNCOMM 
cc YW: We do initialisation once and send it to 
...
!     pressure
      if (if3d) then            ! #2D
         ! This is to have the same number of samples as in the DNS
!     NOTE: Change parameter istcount in time_series.f !!!
         if (mod(ISTEP,stat_comp).eq.0) then
cc  UNCOMM FOR STATS
            call stat_pts_compute(tmpvel,slvel,tmppr)  !UNCOMM 
            ! print *, "YW: stat_pts_compute END"
         endif
      endif
