...
!     x_init and delta x
      if(NIO.eq.0) write(6,*) 'reading stat_pts3d.in'

      ierr = 0
      if (ifascii) then
!     text file
      !    if(NID.eq.0) then
            open(50,file='stat_pts3d.in',status='old',err=100)
            read(50,*,err=100) pts_npt2d, pts_nlev, pts_dir
            pts_npt2d = pts_npt2d
! SKIP 2D point XY    
            read(50,*,err=100) pts_xmin, pts_dx
            do il=1,pts_npt2d
                  read(50,*,err=100) vdum(1), vdum(2)
            enddo
! load z    
            do il = 1,pts_nlev
                  read(50,*,err=100) uniqz(il)
            enddo
 ...
 101        continue
      !    endif
         ...
         print *, "YW: Plase provide ASCII file!"

      endif                     ! ifascii

      print *, "NID",NID, "Z:",uniqz(1:pts_nlev)
...
!     generate whole 3D distribution
     ...
     $        pts_ndx,pts_nyi,pts_ndy,pts_xmin,pts_dx,uniqz)
...
!     the number of failures
      if (NID.eq.0) print *, "YW: inth_hpts",inth_hpts
...
c$$$!     testing
!          write(str,'(i2.2)') NID
!          open(unit=10001,file='pts_fnl.txt'//str)
!          write(10001,*) NID, npoints, npts, LHIS
!          do itl1=1,npts
!             write(10001,*) itl1, ipts(itl1),
!      $           (pts(itl2,itl1),itl2 = 1,LDIM)
!          enddo
!          close(10001)
c$$$!     testing end
...
!     no need for error check
                     open(50,file='stat_pts3d.in',status='old',ERR=400)
...
!     fill missing coordinates based on pts_dir
     ...
     $                           pts_ndx,pts_nyi,pts_ndy,pts_xmin,
     $                           pts_dx,uniqz)
...
!     be changed in the future
      ...
      if(NIO.eq.0) write(6,*) 'YN Check Writting:',icall,ltsnap,istcount   
