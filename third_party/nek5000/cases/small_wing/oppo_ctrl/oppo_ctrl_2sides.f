...
c The control here will be applied on both suction and pressure side 
c Yuning Wang 
c==============================================

c------------------------------------------------------------------
        subroutine oppo_ctrl 
c: Main Subroutine for Initialize the control and compute the velocity fluctuation
c=============================================
c       Define variable
c=============================================
        implicit none 
...
cc YW:
        ...
        ! Defining the control region for both pressure and suction side
        real ymin,ymax 
        parameter(ymin=-0.2)
        parameter(ymax=0.2)
...
c=============================================
        ...
        if (NID.eq.0) then
        print *,"=============================="
        print *, "YW: START INIT WALL NORMAL ANGLE"
        print *,"=============================="
        endif 
c------------------------------------------
c  Step1: Load the extruded field to get the correct angle
        ...
        if (NID.eq.0) print *, "YW: PRE DATA Loaded"
c-----------------------------------------
c------------------------------------------
...
!!!! after maximum chamber and over the pressure side (D)
        ...
        enddo ! ie=1,ntot 
c-----------------------------------------------
        if (NID.eq.0) print *, "YW: WALL NORMAL ANGLE FINISHED"
...
c=============================================
        ...
        !------------------------------------------------------
        ! For calibrate the location of detection plane
        real yp1, yp2                       
        parameter(yp1=15.0) ! before x/c = 0.4 
        parameter(yp2=15.0) ! after x/c = 0.4 
        !------------------------------------------------------
        ! Setup as parameter 
...
c=============================================
        ...
        do il=1,numctrl
                ...
                call X2Utau(xw,yw,utaux)       ! Interploation to achieve the friction velocity
                

cc YW: Modified on 2024.05.06:
c-------------------------------------------------------------------
c               Detection Plane Determination 
c-------------------------------------------------------------------

c------------------------------------------
c  CASE 1        The benchmark case: 
c------------------------------------------
                ynorm = yp1 * nuctrl / utaux 
c------------------------------------------
                ...
                ! On suction side we add, and on pressure side we minus
                ! It is determined in by the inclination angle
                yct = yw + dyy
...
cc Pressure Side
cc---------------------------------------
        pb(1) =	0.15092852189450634
        pb(2) =	-1.8654539677569346
        pb(3) =	16.842787714726455
        pb(4) =	-94.11450436759887
        pb(5) =	333.75783554571217
        pb(6) =	-755.7317598956109
        pb(7) =	1085.7747671566672
        pb(8) =	-956.8272864235888
        pb(9) =	471.58382179883006
        pb(10)= -99.53803730977009
...
c----------------------
     ...
     &           (crdctl(kl,il),kl=1,NDIM)
        ...
        enddo
        if (NID.eq.0) print *,"YW: REGISTERED SENSING PLANE !"
...
c----------------------------------
cc STEP 4: Save Time Series (NEW ADDED)
        call ctrl_pts_out()
c----------------------------------
...
c=============================================
        ...
        integer e,i,j,k ! For iteration
        integer eg,ex,ey,ez ! For reshape 
...
c-----------------------------------------------
c   YW:  A MORE COMPACT WAY TO DO AVERAGE
c-----------------------------------------------
c: Step 1: Compute the Spatial Mean
c-----------------------------------------------
        call z_averaging(ctrlV,avgV)

c-----------------------------------------------
c: Step 2: Substract the mean from reshaping the 2D array 
c-----------------------------------------------
        do e=1,lelt
        ...
        do i=1,lx1
                do k=1,lz1
                ! Directly subtract the mean instead of reshaping avgV first
                ctrlV(i,j,k,e)=ctrlV(i,j,k,e)-avgV(i,j,ex,ey) 
                enddo
        enddo
        enddo
...
c=============================================
        ...
        ! For rearrange the velocity 
        integer totpts
        integer wel,wface,wx,wy,wz ! indicies for the wall
        real vf                    ! Corresponding velocity found
        real dumi 
        parameter (dumi=100)
...
!     Velocity interpolation 
        ...
        call findpts_eval(inth_hpts,vctl(1,1),1,
     &                       rcode,1,
     &                       proc,1,
     &                       elid,1,
     &                       rst,NDIM,numctrl,
...
c--------------------------------------------
c Function for addressing the issue of traversing 
        if (numctrl.gt.0) then 
...
c----------------------------------------
        ...
        call findpts_eval(inth_hpts,vwall(1,1),1,
     &                       rcode,1,
     &                       proc,1,
     &                       elid,1,
     &                       rst,NDIM,totctrl,
