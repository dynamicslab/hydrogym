...
cc YW: The idea is to find the wall points by using the range and the wall boundary condition
     ...
     $              yf.gt.ctrlys .and. yf.lt.ctrlye .and.
     $              xf.eq.xr .and. yf.eq.yr .and. ! Only if the coordinates are at face mid pts 
     $              bcb.eq.'W  ') then
...
c=============================================
        ...
        if (NUMCTRL.gt.0) then 
        print *,"NID=",NID,"YW: Bubble Sorting"
! step 1: Bubble sort 
!-------------------------------------
        ! Bubble sort         
        do il = 1, NUMCTRL-1
...
c=============================================
        ...
        do il=1,numctrl
                ...
                snx = unx(iy,iz,iface,ie) ! Note:  iy,iz
                sny = uny(iy,iz,iface,ie)
                snz = unz(iy,iz,iface,ie)
                elseif (f.eq.3.or.f.eq.4)  then ! "s face"
                snx = unx(ix,iz,iface,ie) !  ix,iz
                sny = uny(ix,iz,iface,ie)
                snz = unz(ix,iz,iface,ie)
                elseif (f.eq.5.or.f.eq.6)  then ! "t face"
                snx = unx(ix,iy,iface,ie) ! ix,iy
                sny = uny(ix,iy,iface,ie)
                snz = unz(ix,iy,iface,ie)
                endif
...
cc Statistics obtained from Marco's database
cc---------------------------------------
        pl(1) =	-0.07884998467503268
        pl(2) =	4.03818278672298
        pl(3) =	-39.09152808439776
        pl(4) =	197.95490108613646
        pl(5) =	-599.6157158145734
        pl(6) =	1141.2219165873776
        pl(7) =	-1375.2400801510703
        pl(8) =	1016.2515710504615
        pl(9) =	-418.6187916900328
        pl(10)=	73.14652919719165
...
c----------------------
     ...
     &           (crdctl(kl,il),kl=1,NDIM)
        ...
        enddo
        print *, "NID=",NID,"YW: SENSING PLANE interpolated!"
...
cc Print in the output 
        if (NIO.eq.0) print *, "YW: VEL FLUCT COMPUTED"
c----------------------------------
...
c=============================================
        ...
        real ctrlV(LX1,LY1,LZ1,LELT,nfldc)
        ...
        real normY(LX1,LY1,LZ1,LELT) ! For sny
        real normX(LX1,LY1,LZ1,LELT) ! For snx
        real normVx(LX1,LY1,LZ1,LELT) ! For sny
        real normVy(LX1,LY1,LZ1,LELT) ! For sny
        real normVn(LX1,LY1,LZ1,LELT) ! For sny
...
c       Function
c=============================================
        nfaces = 2*NDIM ! Num face depends on the dimension of simulation 
        NEL    = NELFLD(IFIELD) ! The number of elements 
        ntot   = LX1*LY1*LZ1*LELT
cc: YW: I write the projection explictly, easier to check the implementation. 
cc: Should be optimized in the further for running quicker.

        ! Iterate through all the elements, the same as we find the wall
        do ie=1,NEL 
        do iface=1,nfaces
                call facind(KX1,KX2,KY1,KY2,KZ1,KZ2,NX1,NY1,NZ1,iface)
                do iz=KZ1,KZ2
                ...
                ! Step 1: Get the velocity components on Cartesian Coordinates
                vxi=vx(ix,iy,iz,ie)
                vyi=vy(ix,iy,iz,ie)
                
                ! Step 2: Find face normal
                !*******************************
                f = eface1(iface) ! face ID 
                if (f.eq.1.or.f.eq.2) then 
                snx=unx(iy,iz,iface,ie) ! Note:  iy,iz
                sny=uny(iy,iz,iface,ie)
                elseif (f.eq.3.or.f.eq.4) then 
                snx=unx(ix,iz,iface,ie)
                sny=uny(ix,iz,iface,ie)
                elseif (f.eq.5.or.f.eq.6) then 
                snx=unx(ix,iy,iface,ie)
                sny=uny(ix,iy,iface,ie)
                endif
                !*******************************
                
                ! Step 3: Project onto the wall-normal direction 
                ! V_n = vx*cos(theta) + vy*sin(theta)
                ctrlV(ix,iy,iz,ie,1)=vxi*snx+vyi*sny
                ! (Test) Step 5*: Write down the face normal 
                normVn(ix,iy,iz,ie)=vxi*snx+vyi*sny
                normVx(ix,iy,iz,ie)=vxi
                normVy(ix,iy,iz,ie)=vyi
                normX(ix,iy,iz,ie)=snx
                normY(ix,iy,iz,ie)=sny
...
c        call outpost(normX,normY,vz,pr,t,'ang')
c        !Write down the velocity components and the current results 
c        call outpost(vx,vy,normVn,pr,t,'wnv')
c$$$ TEST End
...
c=============================================
        ...
        real ctrlV(LX1,LY1,LZ1,LELT,nfldc),
     $   avgV(LX1,LY1,xnel,ynel), velV(LX1,LY1,LZ1,LELT)
...
c-----------------------------------------------
        ...
        call copy(velV(1,1,1,1),ctrlV(1,1,1,1,1),ntot)
...
c-----------------------------------------------
        ...
        call z_averaging(velV,avgV) ! Modified Avg Subroutine 
        call z_avg_reshape(velV,avgV) ! Now the avgV is transposed and pass the value to velV
...
c-----------------------------------------------
        do ll=1,LELT
                ...
                do il=1,LX1               
                ctrlV(il,jl,kl,ll,1)=
     $                  ctrlV(il,jl,kl,ll,1) ! Pass back to the array
     $                  -velV(il,jl,kl,ll)
                enddo
                enddo
                enddo
        ...
        call copy(Vfluct(1,1,1,1),ctrlV(1,1,1,1,1),ntot) 
...
c=============================================
        ...
        integer itimes 
        COMMON / CTRL / postV,postC,postF,itimes
...
! idea: We find them by pre-stored information
     ...
     $             cz.eq.iz) then
                        ...
                        ! Step 1: Get info:
                        iloc   = il             ! Record the index
                        isfind = .TRUE.         ! Switch Flag
                        vf=vctl(1,iloc)         ! Get Wall-normal Vel

c$$                        ! For test the implementation
                        !---------------------------
c                        itimes = itimes+1       ! Counting 
c                        postV(1,itimes)=-vf     ! Pass Wall-normal Vel
c                        do ilx=1,NDIM           ! Pass coordinates of points
c                        postC(ilx,itimes)=crdwall(ilx,iloc)
c                        enddo
c                        do ilx=1,5
c                        postF(ilx,itimes)=grdwall(ilx,iloc)
c                        enddo
c$$                     !---------------------------
...
c----------------------------------------
        ...
        call findpts_eval(inth_hpts,vwall(ifld,1),nfldc,
     &                       rcode,1,
     &                       proc,1,
     &                       elid,1,
     &                       rst,NDIM,totctrl,
