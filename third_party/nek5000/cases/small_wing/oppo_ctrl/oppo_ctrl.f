...
cc YW: The idea is to find the wall points by using the range and the wall boundary condition
        ...
        do iface=1,nfaces
                ...
                yf=ym1(ix,iy,iz,ie)
                zf=zm1(ix,iy,iz,ie)        
                if (xr.ge.ctrlxs .and. xr.le.ctrlxe .and.! within range of ctrl  Reth
     $              yr.ge.ctrlys .and. yr.le.ctrlye .and.
     $              bcb.eq.'W  ') then
...
c------------------------------------------
c CASE 2     Calibrate the detction plane after maximum chamber (x/c=0.4) to overcome the strong APG
c------------------------------------------
                if (xw.le.0.4) then 
                        ynorm = yp1*nuctrl/utaux ! compute the norm between ctrl and sensing point
                else 
                        ynorm = yp2*nuctrl/utaux
                endif 
c-----------------------------------------
