...
#ifdef BDFD
      integer iel  ! local indices 
      real bdfy    ! imposed body force 
      iel = gllel(ieg)
#endif 
...
#ifdef BDFD 
c===============
cc Update Body Force by damping the vertical velocity 
c===============
      call bodyforce_damping(ix,iy,iz,iel,bdfy)
      ffx = 0.0
      ffy = bdfy 
      ffz = 0.0
#endif 
...
!-----------------------------------------------------------------------
      ...
      uy = -1 * amp * vf 
      uz = 0.0
      ! print *, NID,"[ACT],x,y,z,v",x,y,z,uy
      endif

      endif !if  (y.ge.1.7 .or. y.le.0.3) then
#endif
...
!     Initialization 
      real viscosity, Ubar, Ret, channelh, PII
...
!     Test 
      ...
      betap = 2.0*PII*(1.0/100.0);
      alphap = 2.0*PII*(1.0/250.0);
      eps = Ubar/100.0;      
      ...
      ! For closed channel ==> full height
...
!-----------------------------------------------------------------------
      ...
      integer iel
#ifdef BLOW
      integer ie,iface,nfaces,numctrl
      character*3 bcb
#endif 
      ! c     mesh stretching
!       real Betax, Betay
...
!       Betax = 2.0 ! Not used
!       Betay = 2.4 ! Progressive ratio for y
!       !Domian size 
!       Lx = 2.67    
!       Ly = 2.    ! this should be 2xh since we are full channel
!       Lz = 0.8  !
!       W  = 1.929
!       H = 1.
      
c     Transform box mesh to channel (only for the conforming mesh!!)
      call box2phill
...
c     mark faces for object definition
      ...
      enddo ! ie = 1, NEL
      print *, "[NEK] RANK,NUMCTRL",NID,numctrl
#endif
...
cc Initializing the Body-force damping
#ifdef BDFD
      call init_bodyforce
#endif 
...
!      $    tanh(Betay) + 1.0)
      ym1(i,1,1,1)=ymax-(tanh(Betay*(1.0-ym1(i,1,1,1)))/tanh(Betay))
      enddo
