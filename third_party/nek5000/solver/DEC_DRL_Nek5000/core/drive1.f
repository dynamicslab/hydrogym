...
1998  call drl_if_restart(drl_check_s,2)
      lastep = 0 ! Reset the steper
      if (NID.eq.0) print *,"[NEK] Back to SOLV,init:",drl_check_s
      call drl_chkpt_reset() ! Reload the restart file 
     ...
     &     ' running solver in post processing mode'
      ...
      do kstep=1,nsteps,msteps
        ...
        drl_check_s = .FALSE.
        call userchk
        call drl_if_restart(drl_check_s,0)
        if (NID.eq.0) print *, "[NEK] IF QUIT",drl_check_s
        call nekgsync()
        if (drl_check_s) then 
        if (NID.eq.0) print *, "[NEK] QUIT SOLV LOOP"
          lastep = 1
        endif
        !-------------------------
