...
c-----------------------------------------------------------------------
      ...
      logical nopen_init
      save    nopen_init
      data    nopen_init / .false. /

      call blank(fname,132)      !  zero out for byte_open()
      
      ! Reset file counters only once per run. DRL can set ISTEP=0
      ! mid-run, so guard against resetting counters every time that happens.
      !-----------------
      if (ISTEP.eq.0 .and. .not.nopen_init) then 
        do iyw=1,99
          nopen(iyw,1)=0
          nopen(iyw,2)=0
        enddo 
