...
C     and path (PATH_MULT(n-1))
      ...
      print *, "[NEK] AT MULTIMESH"
      ! YW: Ignore here since it does not make sens 
      call mpi_initialized(mpi_is_initialized, ierr) !  Initialize MPI
      print *, "[NEK] MPI IS INIT",mpi_is_initialized
      if ( mpi_is_initialized .eq. 0 ) call mpi_init (ierr)
      
      call mpi_comm_dup(mpi_comm_world,intracomm,ierr)
      print *, "MPI_COMM_DUP DONE"
      call iniproc(intracomm)
      print *, "INI PROC"
      
      call mpi_comm_size(intracomm,np_global,ierr)
      call mpi_comm_rank(intracomm,nid_global,ierr)
